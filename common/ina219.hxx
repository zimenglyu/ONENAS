...
struct INA219Stats {
    ...
    int32_t sample_count;  // readings taken inside the window
};
...
class INA219Sampler {
   public:
    typedef function<bool(INA219Reading&)> Reader;

    INA219Sampler() : running(false), sample_interval_us(1000) {
    }
    ...
    void set_sample_interval_us(int32_t interval_us) {
    ...
    bool start(Reader reader) {
        ...
        reader = reader;
        {
            lock_guard<mutex> lock(readings_mutex);
            readings.clear();
        }
        running = true;
        worker = thread(&INA219Sampler::sample_loop, this);
        return true;
    }
    ...
    INA219Stats get_stats(int64_t window_start_us, int64_t window_end_us) const {
        ...
        vector<const INA219Reading*> in_window;
        for (const INA219Reading& r : readings) {
        ...
        if (in_window.empty()) {
            ...
            for (const INA219Reading& r : readings) {
                if (r.timestamp_us < window_start_us) {
                    before = &r;
                }
                if (r.timestamp_us > window_end_us && after == nullptr) {
                    after = &r;
                }
            }
            if (before != nullptr) {
                in_window.push_back(before);
            }
            if (after != nullptr) {
                in_window.push_back(after);
            }
        }
    ...
    INA219Stats get_stats() const {
        int64_t first, last;
        {
            ...
            first = readings.front().timestamp_us;
            last = readings.back().timestamp_us;
        }
   ...
   private:
    ...
    thread worker;
    mutable mutex readings_mutex;
    vector<INA219Reading> readings;
    ...
    void sample_loop() {
        while (running) {
            ...
            int32_t slept = 0;
            while (running && slept < sample_interval_us) {
                int32_t slice = min(1000, sample_interval_us - slept);
