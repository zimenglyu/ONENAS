...
class Log {
   private:
    ...
    /**
     * Defaults at -1, when set to a process rank (>= 0) the Log will only print messages if its rank is the same as the
     * restricted rank. This is useful, for allowing initialization messages to only be printed out by a single process.
     */
     ...
     * The Log::set_thread_id(int32_t) mehod needs write access so it will use the std::shared_mutex::lock()x and
     * std::shared_mutex()::unlock() mechanism, while the Log::get_thread_id() only needs read access and can used the
     * std::shared_mutex::lock_shared() and std::shared_mutex::unlock_shared() methods.
     */
   ...
   public:
     ...
     * following command line arguments and instructions:
