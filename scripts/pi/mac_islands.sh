#!/bin/bash
# Run on a Mac on the pi's network:   sh scripts/pi/mac_islands.sh
# The pinned 40-island best run on one panel-set, streaming the best genome of EVERY island (the pi also scores their ensemble)
