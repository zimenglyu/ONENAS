...
class OneNasIslandSpeciationStrategy : public SpeciationStrategy {
    ...
    public:
      ...
      */
     void write_global_best_prediction(
      ...
      */
     bool calculate_prediction_performance(
         const vector<vector<vector<double> > >& predictions, const vector<vector<vector<double> > >& test_output,
         double& naive_mse, double& genome_mse
     );
      ...
      * \param test_output the test output data
      */
     void write_prediction_file(
         const string& filename, const vector<vector<vector<double> > >& predictions,
         const vector<vector<vector<double> > >& test_input, const vector<vector<vector<double> > >& test_output
     );

     /**
      * Writes every island's elite predictions to generation_<g>_elites.csv as
      * island,elite_rank,stock,row,predicted. elite_rank 0 is the island's best and `row`
      * joins to generation_<g>_global_best.csv. Only written with --write_elite_predictions.
      */
     void write_elite_prediction_file(
         int32_t current_generation, const vector<vector<vector<double> > >& test_input,
         const vector<vector<vector<double> > >& test_output
     );
     ...
     void finalize_generation(
         int32_t current_generation, const vector<vector<vector<double> > >& validation_input,
         const vector<vector<vector<double> > >& validation_output, const vector<vector<vector<double> > >& test_input,
         const vector<vector<vector<double> > >& test_output
     ) override;

     // New PER-specific method that returns elite genomes
     vector<RNN_Genome*> finalize_generation_with_genomes(
         int32_t current_generation, const vector<vector<vector<double> > >& validation_input,
         const vector<vector<vector<double> > >& validation_output, const vector<vector<vector<double> > >& test_input,
         const vector<vector<vector<double> > >& test_output
     );

     void evaluate_elite_population(
         const vector<vector<vector<double> > >& validation_input,
         const vector<vector<vector<double> > >& validation_output
     );
